...
ListCurrentOrders::ListCurrentOrders(
    ...
    const greentop::sport::OrderProjection& orderProjection,
...
wxThread::ExitCode ListCurrentOrders::Entry() {
    ...
    event->SetPayload<greentop::sport::CurrentOrderSummaryReport>(currentOrderSummaryReport);
    QueueEvent(event);

    return (wxThread::ExitCode) 0;
}

greentop::sport::CurrentOrderSummaryReport ListCurrentOrders::DoListCurrentOrders() {
    ...
    greentop::sport::ListCurrentOrdersRequest lcor(std::set<std::string>(), marketIds, orderProjection);
